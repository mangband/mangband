...
static errr textTermHook(int x, int y, int n, byte attr, cptr s) {
	...
	float r = fmin((float)td->cell_w / (float)w, (float)td->cell_h / (float)h);
	if (r < 1.0) {
		w *= r;
		h *= r;
	}
	
	int offsetx = (td->cell_w / 2) - (w/2);
	int offsety = (td->cell_h / 2) - (h/2);
...
static errr pictTermHook(int x, int y, byte attr, char ch) {
	...
	SDL_Rect cell_rect = { x*(td->cell_w), y*(td->cell_h), td->cell_w, td->cell_h };
	SDL_Rect sprite_rect = { col*(td->pict_data->w), row*(td->pict_data->h), td->pict_data->w, td->pict_data->h };
	SDL_RenderDrawRect(td->renderer, &cell_rect);
...
errr renderMenu(TermData *td) {
	...
	int width = td->cell_w * (td->rows-8);	// menu width
	int height = td->cell_h * (td->cols-2);	// menu width
	SDL_Rect menu_rect = { td->cell_w*4, td->cell_h, width, height };
	// draw background
...
errr ttfToFont(FontData *fd, cptr filename, int fontsize, int smoothing) {
	...
	TTF_Font *font = TTF_OpenFont(buf, fontsize);
	if (!font) {
